...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const login = async (email: string, password: string) => {
    ...
    const response = await fetch('http://solarmetrics-backend.brazilsouth.cloudapp.azure.com:8080/auth', {
      method: 'POST',
      headers: {
  ...
  const register = async (data: RegisterData) => {
    const response = await fetch('http://solarmetrics-backend.brazilsouth.cloudapp.azure.com:8080/cliente', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
